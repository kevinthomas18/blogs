...
export default function EditBlogPage({ blog }) {
  const { slug } = useParams();
  const { data: session } = useSession();
  const router = useRouter();

  const [title, setTitle] = useState(blog.title);
  const [topDescription, setTopDescription] = useState(blog.top_description);
  const [bottomDescription, setBottomDescription] = useState(
    blog.bottom_description
  );
  const [sections, setSections] = useState(
    blog.sections || [{ heading: "", content: "" }]
  );
  const [files, setFiles] = useState(null);
  const [isPublished, setIsPublished] = useState(blog.is_published);
  const [premium, setPremium] = useState(blog.premium);
